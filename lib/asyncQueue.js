...
class AsyncQueue {
	constructor(opts = {}) {
		...
		this[PROPS].enableFileLock = opts.enableFileLock || false;             // 是否启用文件锁
		this[PROPS].fileLockPath = opts.fileLockPath || "";                   // 文件锁目录
	}
	...
	async [EXEC_FN](task) {
		...
		if (props.enableFileLock) {
			...
			dirname = _path.join(fileLockPath, base64Key);
