...
class AsyncQueue {
	constructor(opts = {}) {
		...
		this[PROPS].enableFileLock = false;             // 是否启用文件锁
		this[PROPS].fileLockDir = "";                   // 文件锁目录
	}

	// 使用目录锁
	async [LOCK_FN](dirname, timeout = 0) {
		console.log(`lock ${dirname}`);
	...
	async [EXEC_FN](task) {
		...
		if (props.enableFileLock) {
			const fileLockDir = props.fileLockDir || "";
			const base64Key = base64.encode(task.key);

			dirname = _path.join(fileLockDir, "lock~" + base64Key);

			const ok = await this[LOCK_FN](dirname, task.timeout || queue.timeout || props.timeout);
